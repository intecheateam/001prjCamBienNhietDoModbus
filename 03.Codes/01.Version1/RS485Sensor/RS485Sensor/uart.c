...
void UART_SetBaudRate(uint32_t v_baudRate_u32)
{
	...
	UBRRL = LOBYTE(RegValue);
	UBRRH = HIBYTE(RegValue);
}
 ...
 * Return value	: unsigned char: Number of chars received.
...
/***************************************************************************************************
void UART_TxNumber(unsigned char v_numericSystem_u8, uint32_t v_number_u32, unsigned char v_numOfDigitsToTransmit_u8)
 ***************************************************************************************************
 * I/P Arguments: 
                  unsigned char :  specifies type of number C_BINARY_U8(2),C_DECIMAL_U8(10), C_HEX_U8(16)
                  uint32_t: Number to be transmitted on UART.
                  unsigned char : Number of digits to be transmitted
...
void UART_Printf(const char *argList, ...)
{
	...
	char *str;
	char  ch;
	unsigned char v_numOfDigitsToTransmit_u8;
#if (Enable_UART_TxFloatNumber==1)
